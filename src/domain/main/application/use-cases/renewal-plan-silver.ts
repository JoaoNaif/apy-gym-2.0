...
interface RenewalPlanSilverUseCaseRequest {
  userId: string
  planDuration: 'MENSAL' | 'SEMESTRAL' | 'ANUAL'
}

type RenewalPlanSilverUseCaseResponse = Either<
  ResourceNotFoundError | PlanNotExistError,
  null
>

export class RenewalPlanSilverUseCase {
  constructor(
  ...
  async execute({
    userId,
    planDuration,
  }: RenewalPlanSilverUseCaseRequest): Promise<RenewalPlanSilverUseCaseResponse> {
